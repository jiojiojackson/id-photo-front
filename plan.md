...
**Lightning 容器不配置上述项目环境变量。** Lightning 是无状态 GPU Worker。

`LIGHTNING_API_URL` 不需要在 Lightning 应用内部配置；Vercel 直接读取平台提供的 URL。

### Worker Credential

Vercel 在“开始处理”时生成短期随机 Worker Credential：
...
Neon：只保存 hash
 ↓
Lightning wake body：短期 credential
 ↓
Lightning：仅驻留内存
 ↓
...
Lightning 不保存长期 Bridge 凭证，也不需要 Database/R2/Queue 凭证。
...
6. Vercel 调用 `LIGHTNING_API_URL`，使用 `LIGHTNING_API_KEY` 作为 Lightning 平台外部认证。
7. 请求 body 传递：
   - `worker_run_id`
   - `bridge_url`
   - `worker_credential`
   - `worker_credential_expires_at`
8. Lightning 应用只使用请求中收到的 credential，不需要项目环境变量。
...
1. 确认最新 Vercel Preview Build 与 migration 成功。
2. 验证 Lightning 公网 wake endpoint 能正确接收并转发 Vercel wake payload。
3. 真实执行 3 Job 串行端到端测试：提交 → Queue → 开始 → Lightning → claim → R2 → completed。
4. 测试重复点击开始、Worker 崩溃、lease 到期、重复 complete、单 Job fail/retry。
5. 测量真实 Lightning p95 / 最大推理时间。
6. 根据实际数据校准 lease、heartbeat、presigned URL 有效期。
7. 验证前后端生产链路后，合并后端 Draft PR #2 和前端分支到 `main`。
